 ...
 * ─────────────────────────────────────────────────────
 * FIXES aplicados:
 * - init(fromRestore) → si fromRestore=false limpia solves/currentSolve
 *   explícitamente; si true preserva los valores ya cargados.
 * - Logger integrado en todos los eventos clave:
 *     · solve completado
 *     · DNF automático por timeout de inspección (warn)
 *     · penalidad aplicada/quitada
 *     · revisión de solve anterior
 *     · abandono
 * - Storage.updateParticipantSession al finalizar o abandonar
 *   para registrar cuántos minutos pasó el competidor en el timer.
 */
...
const Timer = (() => {
  ...
  // Timestamp de cuando el competidor entró al timer (para calcular minutos)
  let _timerEnteredAt = null;

  /* ── Init ───────────────────────────────────────────── */
  /**
   * @param {boolean} fromRestore - true cuando se restaura sesión guardada.
   *   Si false (flujo nuevo), limpia solves y currentSolve para evitar
   *   que datos de una categoría anterior contaminen la nueva sesión.
   */
  function init(fromRestore = false) {
    if (!fromRestore) {
      // Flujo completamente nuevo — limpiar cualquier rastro de sesión anterior
      AppState.solves       = [];
      AppState.currentSolve = 0;
    }
    // Si fromRestore = true, AppState.solves y currentSolve
    // ya vienen cargados desde SessionStore.tryRestore()
    ...
    // Registrar cuándo entra al timer
    _timerEnteredAt = Date.now();
    ...
    document.getElementById('sb-cat').textContent =
      catData ? catData.name : AppState.contestant.category;
    ...
    if (AppState.currentSolve >= _totalSolves()) {
      ...
      document.getElementById('solve-badge').textContent =
        `SOLVE ${AppState.currentSolve + 1}`;
      document.getElementById('current-solve-num').textContent =
        AppState.currentSolve + 1;
    }
  }

  /* ── Helpers de duración ────────────────────────────── */
  function _minutesInTimer() {
    if (!_timerEnteredAt) return 0;
    return Math.round((Date.now() - _timerEnteredAt) / 60000);
  }
  ...
  function togglePenalty(type) {
    ...
    const previous = AppState.currentPenalty;
    AppState.currentPenalty = (AppState.currentPenalty === type) ? null : type;

    Logger.info('penalty_toggled', {
      penaltyType: AppState.currentPenalty,
      previous,
      solveMs:     AppState.timerValue,
      display:     msToDisplay(AppState.timerValue),
    });
  ...
  function openReview(idx) {
    ...
    Logger.info('review_opened', {
      reviewSolveNum: idx + 1,
      currentPenalty: solve.penalty || null,
      display:        formatSolve(solve),
    });
  ...
  function reviewTogglePenalty(type) {
    ...
    const previous = solve.penalty;
    solve.penalty  = (solve.penalty === type) ? null : type;

    Logger.info('review_penalty_toggled', {
      reviewSolveNum: reviewIndex + 1,
      penaltyType:    solve.penalty,
      previous,
      display:        formatSolve(solve),
    });
  ...
  /* ── Abandonar ──────────────────────────────────────── */
  function abandon() {
    // Loguear abandono ANTES de limpiar el estado
    Logger.warn('contest_abandoned', {
      solvesCompleted: AppState.solves.length,
      totalSolves:     _totalSolves(),
      minutesInTimer:  _minutesInTimer(),
      solves:          AppState.solves.map(s => ({
        ms:      s.ms,
        penalty: s.penalty || null,
        display: formatSolve(s),
      })),
    });

    // Registrar tiempo en timer si hay participante
    if (AppState.contestant) {
      Storage.updateParticipantSession(
        AppState.contestant.email,
        AppState.contestant.category,
        {
          timerMinutes:  _minutesInTimer(),
    ...
    // Limpiar TODO el estado del competidor
    AppState.solves       = [];
    AppState.currentSolve = 0;
  ...
  function _startInspection() {
    ...
    inspectionInterval = setInterval(() => {
      ...
      if (elapsed >= PLUS2_END_MS) {
        // ── DNF automático por pasar los 17s ──────────────
        _clearInspection();
        _clearHold();

        const scramble = AppState.contest.categories[AppState.contestant?.category]
                           ?.scrambles?.[AppState.currentSolve] || null;

        // Log WARN — esto es lo que causa los DNFs "misteriosos"
        Logger.warn('dnf_inspection_timeout', {
          solveNum:      AppState.currentSolve + 1,
          scramble,
          note: 'El competidor dejó pasar los 17 segundos de inspección sin iniciar el timer',
        });
  ...
  function _startTimer() {
    ...
    // Activar overlay de stop en móvil
    ...
    document.getElementById('timer-hint').textContent =
      'Tocá para detener · Cualquier tecla detiene';
  ...
  function _next() {
    const solve = { ms: AppState.timerValue, penalty: AppState.currentPenalty };

    // Loguear cada solve completado
    Logger.info('solve_completed', {
      solveNum: AppState.currentSolve + 1,
      ms:       solve.ms,
      penalty:  solve.penalty || null,
      display:  formatSolve(solve),
      scramble: AppState.contest.categories[AppState.contestant?.category]
                  ?.scrambles?.[AppState.currentSolve] || null,
    });
...
></twisty-player>
  ...
  function _resetDisplay() {
    ...
    document.getElementById('timer-hint').textContent  =
      'Presioná "Ver Scramble" para continuar';
    document.getElementById('solve-badge').textContent =
      `SOLVE ${AppState.currentSolve + 1}`;
    document.getElementById('current-solve-num').textContent =
      AppState.currentSolve + 1;
  ...
  function _showFinalSummary() {
    ...
    // Registrar tiempo total en el timer al finalizar todos los solves
    if (AppState.contestant) {
      const mins = _minutesInTimer();
      Storage.updateParticipantSession(
        AppState.contestant.email,
        AppState.contestant.category,
        {
    ...
    AppState.solves.forEach((s, i) => {
      ...
      row.innerHTML = `
        <span class="label">Solve ${i + 1}</span>
        <span class="value">${formatSolve(s)}</span>`;
    ...
    bestRow.innerHTML = `
      ...
      <span class="value" style="color:var(--accent)">${getBestDisplay()}</span>`;
    summary.appendChild(bestRow);
    const ao5Row = document.createElement('div');
    ao5Row.className = 'final-stat';
    ao5Row.innerHTML = `
      <span class="label">${getStatLabel()}</span>
      <span class="value" style="color:var(--accent3)">${getAo5Display()}</span>`;
